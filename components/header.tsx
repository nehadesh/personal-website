  ...
  return (
    <header className="z-[999] relative">
      <motion.div
        ...
        rounded-none border-[0.2rem] border-indigo-900 border-opacity-40 bg-white 
        bg-opacity-70 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] 
        ...
        >
          {links.map((link) => (
            ...
            >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-indigo-900 hover:scale-110 transition",
                  { "text-indigo-950": activeSection === link.name }
                )}
                href={link.hash}
                onClick={() => {
